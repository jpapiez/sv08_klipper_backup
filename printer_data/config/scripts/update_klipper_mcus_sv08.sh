...
HOSTSERIAL='stm32f103xe_32FFD705334E563631672051-if00'
...
TOOLHEADSERIAL=('stm32f103xe_37FF68063042413848280643-if00')
...
flash_host(){
	...
	cd ~/klipper/scripts/ && python3 -c 'import flash_usb as u; u.enter_bootloader("/dev/serial/by-id/usb-Klipper_'$HOSTSERIAL'")'
	sleep 3
	~/katapult/scripts/flashtool.py -f ~/klipper/host_mcu_klipper.bin -d /dev/serial/by-id/usb-katapult_$HOSTSERIAL
}
...
flash_toolhead(){
	...
	do
		...
		cd ~/klipper/scripts/ && python3 -c 'import flash_usb as u; u.enter_bootloader("/dev/serial/by-id/usb-Klipper_'$serial'")'
		sleep 3
		~/katapult/scripts/flashtool.py -f ~/klipper/toolhead_mcu_klipper.bin -d /dev/serial/by-id/usb-katapult_$serial
